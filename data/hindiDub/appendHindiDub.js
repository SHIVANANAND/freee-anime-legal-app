...
const JSON_PATH = path.join(__dirname, "hindiDub.json");
...
(async () => {
  console.log("📌 Loading hindiDub.json...");
  const json = JSON.parse(fs.readFileSync(JSON_PATH, "utf8"));

  if (!json.hindiDub) json.hindiDub = [];
  ...
  for (const url of urls) {
    ...
    // Avoid duplicates
    const exists = json.hindiDub.some(p => p.link === data.link);
    if (exists) {
      console.log("⚠️ Already exists in JSON. Skipping.");
      continue;
    }

    json.hindiDub.push(data);
    console.log("✅ Added.");
  }

  // Save updated JSON
  fs.writeFileSync(JSON_PATH, JSON.stringify(json, null, 2));
  console.log("\n💾 Saved updated hindiDub.json");
})();