...
const JSON_PATH = path.join(__dirname, "englishDub.json");
...
(async () => {
  console.log("📌 Loading englishDub.json...");
  const json = JSON.parse(fs.readFileSync(JSON_PATH, "utf8"));

  if (!json.englishDub) json.englishDub = [];
  ...
  for (const url of urls) {
    ...
    // Avoid duplicates
    const exists = json.englishDub.some(p => p.link === data.link);
    if (exists) {
      console.log("⚠️ Already exists in JSON. Skipping.");
      continue;
    }

    json.englishDub.push(data);
    console.log("✅ Added.");
  }

  // Save updated JSON
  fs.writeFileSync(JSON_PATH, JSON.stringify(json, null, 2));
  console.log("\n💾 Saved updated englishDub.json");
})();